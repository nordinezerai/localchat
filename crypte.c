...
int table_permutation_init_block[] = {58,50,42,34,26,18,10,2,
...
int table_permutation_sous_cle[] = {14,17,11,24,1,5,
								 	3,28,15,6,21,10,
								 	23,19,12,4,26,8,
								 	16,7,27,20,13,2,
								 	41,52,31,37,47,55,
...
int bytes_de_decalage_par_cle[] = {-1,1,1,2,2,2,2,2,2,1,2,2,2,2,2,2,1};

int table_expansion_block[] =  {32,1,2,3,4,5,
							 	4,5,6,7,8,9,
							    8,9,10,11,12,13,
							    12,13,14,15,16,17,
							    16,17,18,19,20,21,
...
int table_permutation_block_partie_droite[] = {16,7,20,21,
											   29,12,28,17,
											   1,15,23,26,
											   5,18,31,10,
											   2,8,24,14,
...
int table_permutation_final[] =  {40,8,48,16,56,24,64,32,
...
void generer_sous_cle(char* cle, sous_cle* sous_cles) {
	int i, j;
	int decalage;
	...
	for (i=0; i<56; i++) {
		decalage = table_permutation_init_cle[i];
	...
	for (i=1; i<17; i++) {
		...
		decalage = bytes_de_decalage_par_cle[i];
		if (decalage == 1){
			octet_decalage = 0x80;
		} else {
			octet_decalage = 0xC0;
		...
		for (j=0; j<48; j++) {
			decalage = table_permutation_sous_cle[j];
			if (decalage <= 28) {
...
char* crypter(char* message) {
	...
	generer_sous_cle(cle, sous_cles);
	...
	while(i<total_block) {
			...
			for (j=0; j<64; j++) {
				decalage = table_permutation_init_block[j]; // decalage = 58 ; octet_decalage = 01000000
				octet_decalage = 0x80 >> ((decalage - 1)%8);
			...
			for (k=1; k<=16; k++) {
				...
				for (j=0; j<48; j++) {
					decalage = table_expansion_block[j];
					octet_decalage = 0x80 >> ((decalage - 1)%8);
				...
				droite_etendue_substituee[0] |= ((char)S1[ligne*16+colonne] << 4);
				...
				droite_etendue_substituee[0] |= (char)S2[ligne*16+colonne];
				...
				droite_etendue_substituee[1] |= ((char)S3[ligne*16+colonne] << 4);
				...
				droite_etendue_substituee[1] |= (char)S4[ligne*16+colonne];
				...
				droite_etendue_substituee[2] |= ((char)S5[ligne*16+colonne] << 4);
				...
				droite_etendue_substituee[2] |= (char)S6[ligne*16+colonne];
				...
				droite_etendue_substituee[3] |= ((char)S7[ligne*16+colonne] << 4);
				...
				droite_etendue_substituee[3] |= (char)S8[ligne*16+colonne];
				...
				for (j=0; j<32; j++) {
					decalage = table_permutation_block_partie_droite[j];
					octet_decalage = 0x80 >> ((decalage - 1)%8);
			...
			for (i=0; i<64; i++) {
				decalage = table_permutation_final[i];
				octet_decalage = 0x80 >> ((decalage - 1)%8);
